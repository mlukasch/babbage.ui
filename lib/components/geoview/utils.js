...
function getOrdinalColorScale() {
  if (!ordinalColorScale) {
    var categoryColors = ['#CF3D1E', '#F15623', '#F68B1F', '#FFC60B', '#DFCE21', '#BCD631', '#95C93D', '#48B85C', '#00833D', '#00B48D', '#60C4B1', '#27C4F4', '#478DCB', '#3E67B1', '#4251A3', '#59449B', '#6E3F7C', '#6A246D', '#8A4873', '#EB0080', '#EF58A0', '#C05A89'];
    ordinalColorScale = d3.scale.ordinal().range(categoryColors);
  }
...
function getLinearColorScale(color) {
  var c = d3.hsl(color);
  c.l = 0.925;
  return d3.scale.linear().domain([0.0, 1.0]).range([c, color]);
}
...
function calculateCenter(dimensions) {
  return [(dimensions.left + dimensions.right) / 2, (dimensions.top + dimensions.bottom) / 2];
}

function calculateScale(pathBounds, viewportBounds) {
  return Math.min(viewportBounds.width / Math.abs(pathBounds[0][0] - pathBounds[1][0]), viewportBounds.height / Math.abs(pathBounds[0][1] - pathBounds[1][1]));
}
...
function createPath(options) {
  ...
  .scale(1).translate([options.width / 2, options.height / 2]);
...
function updateValues(geoJson, options) {
  ...
  var scale = d3.scale.linear().domain(range).range([0, 1]).clamp(true);
...
function formatAmount(value, currencySign) {
  var suffixes = [[1000000000, ' Billions'], [1000000, ' Millions'], [1000, ' Thousands']];
  var suffix = '';
...
function setSelection(datum, options) {
  return {
    ...
    className: function className(d) {
      if (d == datum) {
        return 'selected';
      }
      return '';
    ...
    strokeColor: function strokeColor(d) {
      if (!datum || d == datum) {
    ...
    textPosition: function textPosition(d) {
      return 'translate(' + options.path.centroid(d) + ')';
    },
    textFill: function textFill(d) {
      if (!datum || d == datum) {
        return '#272727';
      }
      return '#7f7f7f';
    },
    textSize: function textSize(d) {
    ...
    currencySign: function currencySign(value) {
...
function generateValueRanges(range, stepCount) {
  ...
  var scale = d3.scale.linear().domain([0, 1]).range(range);
  var result = [];
