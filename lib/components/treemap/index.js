  ...
  _createClass(TreeMapComponent, [{
    ...
    value: function build(endpoint, cube, params, wrapper, colorSchema) {
      ...
      api.aggregate(endpoint, cube, params).then(function (data) {
        ...
        root.currency = data.summary["currency"];
        root.summary_fmt = Utils.numberFormat(data.summary[params.aggregates]);
        root.summary_currency = Utils.moneyFormat(root.summary_fmt, root.currency);
        ...
        for (var i in data.cells) {
          ...
          cell._area_currency = Utils.moneyFormat(cell._area_fmt, root.currency);
          ...
          cell._percentage = measure.value && data.summary && params.aggregates ? measure.value / Math.max(data.summary[params.aggregates], 1) : 0;
        ...
        var node = div.datum(root).selectAll(".node").data(that.treemap.nodes).enter().append("a").attr("href", function (d) {
          ...
          return d.children ? null : '<span class=\'amount\'>' + d._area_currency + '</span>' + d._name;
        }).on("click", function (d) {
        ...
        _this2.emit('dataLoaded', that, root);
