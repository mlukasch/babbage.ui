  ...
  _createClass(TreeMapComponent, [{
    ...
    value: function build(endpoint, cube, params, wrapper, colorSchema) {
      ...
      api.aggregate(endpoint, cube, params).then(function (data) {
        ...
        if (hasTextOverflow) {
          var listdiv = d3.select(wrapper).append("div").style("position", "relative").style("width", size.width + "px");

          // Add treemap list:
          var nodetable = listdiv.datum(root).append("table").attr("class", "nodetable");
          nodetable.append("tr").html("<th>title</th><th>amount</th><th>share</th>");
          nodetable.selectAll(".datarow").data(that.treemap.nodes.sort(function (x, y) {
            return x._percentage < y._percentage;
          })).enter().append("tr").attr("class", "datarow").html(function (d) {
            if (d._percentage < 0.02) {
              return '';
            }
            var name = d.href ? '<a href="' + d.href + '">' + d._name + '</a>' : d._name;

            return d.children ? null : '<td><span>' + name + '</span></td><td>' + d._area_fmt + '</td><td>' + (d._percentage * 100).toFixed(2) + '%</td>';
          }).select("td").insert("span", ":first-child").style("background", function (d) {
            return d._color;
          }).attr("class", "colorbox");
        }
